...
namespace TallyJ.CoreModels
{
    public class BallotAnalyzer : DataConnectedModel
    {
            ...
            out string statusCode, out int spoiledCount)
        {
            ...
            var needsVerification = voteInfos.Any(v =>
                v.PersonCombinedInfo.HasContent() &&
                !v.PersonCombinedInfo.StartsWith(v.PersonCombinedInfoInVote ?? "NULL"));
            ...
            // find duplicates
            if (voteInfos.Any(
                vote => voteInfos.Count(v => v.PersonGuid.HasValue && v.PersonGuid == vote.PersonGuid) > 1))
            {
