    ...
    public JsonResult FinalResultsJson
    {
      get
      {
        ...
        return new
        {
          ...
          EnvelopesImported = resultSummaryFinal.ImportedBallots,
    ...
    public object GetCurrentResults()
    {
      try
      {
        ...
        var unprocessedOnlineBallots = _election.OnlineWhenOpen.HasValue
          ? Db.OnlineVotingInfo
            .Join(Db.Voter.Where(p => p.VotingMethod == VotingMethodEnum.Online.Value || p.VotingMethod == VotingMethodEnum.Kiosk.Value), ovi => ovi.PersonGuid, p => p.PersonGuid, (ovi, p) => ovi)
