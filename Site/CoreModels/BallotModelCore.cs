...
namespace TallyJ.CoreModels
{
  public abstract class BallotModelCore : DataConnectedModel, IBallotModel
  {
    ...
    public JsonResult DeleteBallotJson()
    {
      ...
      var hasVotes = Vote.AllVotesCached.Any(v => v.BallotGuid == ballotGuid);
    ...
    public void SetAsCurrentBallot(int ballotId)
    {
      ...
      var ballotInfo = Ballot.AllBallotsCached.SingleOrDefault(b => b.C_RowId == ballotId);
    ...
    public JsonResult SaveVote(int personId, int voteId, int count, Guid invalidReason)
    {
      ...
      if (voteId != 0)
      {
        ...
        var vote = Vote.AllVotesCached.SingleOrDefault(v => v.C_RowId == voteId);
        ...
        var person1 = Person.AllPeopleCached.SingleOrDefault(p => p.C_RowId == personId);
      ...
      if (ok)
      {
        var nextVoteNum = 1 + Vote.AllVotesCached.Where(v => v.BallotGuid == ballot.BallotGuid)
    ...
    {
      var vote = Db.Vote.SingleOrDefault(v => v.C_RowId == vid);
      if (vote == null)
      {
      ...
      var ballotStatusInfo = ballotAnalyzer.UpdateBallotStatus(ballot, VoteInfosForCurrentBallot());
      var isSingleName = UserSession.CurrentElection.IsSingleNameElection;
      var location = Location.AllLocationsCached.Single(l => l.LocationGuid == ballot.LocationGuid);
    ...
    public static int BallotCount(Guid locationGuid, string computerCode, bool isSingleName)
    {
      ...
      if (isSingleName)
      {
        var allBallotGuids = Ballot.AllBallotsCached.Where(b => b.LocationGuid == locationGuid && b.ComputerCode == computerCode)
          .Select(b => b.BallotGuid).ToList();

        sum = Vote.AllVotesCached.Where(v => allBallotGuids.Contains(v.BallotGuid))
          .Sum(vi => vi.SingleNameElectionCount).AsInt();
      }
      else
      {
        sum = Ballot.AllBallotsCached.Count(b => b.LocationGuid == locationGuid && b.ComputerCode == computerCode);
      }
      return sum;
    }
    ...
    public static int BallotCount(Guid locationGuid, bool isSingleName)
    {
      ...
      if (isSingleName)
      {
        var allBallotGuids = Ballot.AllBallotsCached.Where(b => b.LocationGuid == locationGuid)
          .Select(b => b.BallotGuid).ToList();
      ...
      {
        sum = Ballot.AllBallotsCached.Count(b => b.LocationGuid == locationGuid);
      }
      return sum;
    }
    ...
    public Ballot GetCurrentBallot()
    {
      ...
      var ballot = Ballot.AllBallotsCached.SingleOrDefault(b => b.C_RowId == currentBallotId);
    ...
    public List<Vote> CurrentVotes()
    {
      ...
      return Db.Vote.Where(v => v.BallotGuid == ballot.BallotGuid).ToList();
    }
