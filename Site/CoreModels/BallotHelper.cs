...
namespace TallyJ.CoreModels
{
  ...
  {
    public int BallotCount(Guid locationGuid, bool isSingleName, List<Ballot> ballots = null,
    ...
    public int BallotCount(Guid locationGuid, string computerCode, bool isSingleName, List<Ballot> ballots = null,
        List<Vote> votes = null)
    {
      int sum;
      ballots = ballots ?? new BallotCacher(Db).AllForThisElection;
      ...
      {
        var allBallotGuids = ballots
            .Where(b => b.LocationGuid == locationGuid && b.ComputerCode == computerCode)
            .Select(b => b.BallotGuid).ToList();
    ...
    public List<VoteInfo> VoteInfosForBallot(Ballot ballot, List<Vote> allVotes)
    {
      ...
      return (allVotes ?? new VoteCacher(Db).AllForThisElection.ToList())
          ...
          .JoinMatchingOrNull(new PersonCacher(Db).AllForThisElection, v => v.PersonGuid, p => p.PersonGuid,
              (v, p) => new { v, p })
          .Select(g => new VoteInfo(g.v, currentElection, ballot, currentLocation, g.p))
